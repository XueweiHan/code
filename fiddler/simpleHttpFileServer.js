    ...
    static function SimpleHttpFileServer(oSession: Session) {
        ...
        if (oSession.HostnameIs(System.Environment.MachineName) || oSession.HostnameIs('localhost') || oSession.HostnameIs('f.cn')) {
            try {
                ...
                if (path[1] === ':') {
                    if (!oSession.oRequest.headers.Exists("Authorization") || oSession.oRequest['Authorization'] !== 'Basic aGVsbG86d29ybGQ=') {
                        oSession['ui-backcolor'] = 'pink';

                        oSession.utilCreateResponseAndBypassServer();
                        oSession.utilSetResponseBody('<html><body>[File Server] Authentication Required.</body></html>'); 
                        oSession.responseCode = 401;
                        oSession.oResponse['WWW-Authenticate'] = 'Basic realm="[File Server]"';
